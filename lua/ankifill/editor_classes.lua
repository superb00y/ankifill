...
local default_contents = require("ankifill.config").get("default_field_contents")
...
local function create_buf()
	...
	local options = {
		...
		modifiable = true,
		filetype = "html",
		syntax = "html", -- Changed from html.css to html for better compatibility
		bufhidden = "wipe",
	}
...
function Editor:new(model_name, deck)
	...
	for field_name, field in pairs(fields) do
		if field_name ~= "header" then
			vim.api.nvim_buf_set_lines(field.buf, 0, -1, false, { default_contents })
		end
	end
